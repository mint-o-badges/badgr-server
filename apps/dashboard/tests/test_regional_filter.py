...
3. CompetencyService - Competency area caching service
...
from dashboard.services.competency_service import CompetencyService
...
class CompetencyServiceTest(BadgrTestCase):
    """
    Test suite for CompetencyService.

    Tests the caching functionality for competency areas loaded from badge extensions.
    Ensures proper cache behavior and data retrieval.
    """
    ...
    def setUp(self):
        ...
        cache.clear()
        CompetencyService.invalidate_cache()

        # Create test user and issuer
        self.user = BadgeUser.objects.create(
        ...
        # Create badge class with competency extension
        self.badge_class = BadgeClass.objects.create(
            name='IT Badge',
            description='IT skills badge',
            created_by=self.user,
            slug='it-badge',
            issuer=self.issuer,
            criteria_text='Complete IT course'
        )

        # Create competency extension
        self.extension = BadgeClassExtension.objects.create(
    ...
    def test_get_competency_areas(self):
        """
        Test retrieving competency areas from database.
        Verify that DB query works and returns expected data structure.
        """
        # Get competency areas
        areas = CompetencyService.get_competency_areas()

        # Should return a dictionary
        self.assertIsInstance(areas, dict)

        # Should have at least the 'other' fallback category
        self.assertIn('other', areas)

        # Each area should have expected structure
        for area_id, area_data in areas.items():
            self.assertIn('nameKey', area_data)
            self.assertIn('displayName', area_data)
            self.assertIn('color', area_data)

    def test_competency_areas_structure(self):
        """Test that competency areas have correct data structure"""
        areas = CompetencyService.get_competency_areas()

        # Check 'other' category structure
        other = areas['other']
        self.assertEqual(other['nameKey'], 'competency.area.other')
        self.assertEqual(other['displayName'], 'Other')
        self.assertEqual(other['color'], '#492E98')

    def test_cache_behavior(self):
        """
        Test caching behavior.
        Verify that data is cached after first call and retrieved from cache on subsequent calls.
        """
        # Clear cache
        CompetencyService.invalidate_cache()

        # First call should query database
        areas1 = CompetencyService.get_competency_areas()

        # Check that data was cached
        cached_data = cache.get(CompetencyService.CACHE_KEY)
        self.assertIsNotNone(cached_data)
        self.assertEqual(cached_data, areas1)

        # Second call should retrieve from cache
        areas2 = CompetencyService.get_competency_areas()
    ...
    def test_cache_invalidation(self):
        ...
        # Verify cached
        cached = cache.get(CompetencyService.CACHE_KEY)
        ...
        # Getting data again should repopulate cache
        areas2 = CompetencyService.get_competency_areas()
    ...
    def test_cache_timeout(self):
        ...
        # Get data to populate cache
        CompetencyService.get_competency_areas()

        # Verify cache timeout setting
        self.assertEqual(CompetencyService.CACHE_TIMEOUT, 3600)

    def test_competency_area_from_extension(self):
        """Test that competency areas are loaded from badge extensions"""
        CompetencyService.invalidate_cache()

        # Get areas
        areas = CompetencyService.get_competency_areas()

        # Should include area from test extension
        # The area name is normalized: 'IT & Digital' -> 'it_digital'
        self.assertIn('it_digital', areas)

        # Check the area data
        it_digital = areas['it_digital']
        self.assertEqual(it_digital['nameKey'], 'competency.area.it_digital')
        self.assertEqual(it_digital['displayName'], 'IT & Digital')

    def test_multiple_competency_areas(self):
        """Test handling multiple competency areas"""
        # Create another badge with different competency area
        badge_class2 = BadgeClass.objects.create(
            name='Social Skills Badge',
            description='Social competencies badge',
            created_by=self.user,
            slug='social-badge',
            issuer=self.issuer,
            image='badge.png',
            criteria_text='Complete social skills course'
        )

        extension2 = BadgeClassExtension.objects.create(
            badgeclass=badge_class2,
            name='extensions:CompetencyExtension',
            original_json=[{
                'type': ['Extension', 'extensions:CompetencyExtension'],
                'competencyId': 'communication',
                'competencyArea': 'Social Competencies',
                'hours': 3
            }]
        )
        ...
        self.assertIn('it_digital', areas)
        self.assertIn('social_competencies', areas)

    def test_empty_database(self):
        """Test behavior when no competency extensions exist in database"""
        # Delete all extensions
        BadgeClassExtension.objects.all().delete()
        CompetencyService.invalidate_cache()

        # Get areas
        areas = CompetencyService.get_competency_areas()

        # Should still return dict with 'other' category
        self.assertIsInstance(areas, dict)
        self.assertIn('other', areas)

    def test_invalid_json_in_extension(self):
        """Test handling of invalid JSON in extension data"""
        # Create extension with invalid JSON string
        badge_class_invalid = BadgeClass.objects.create(
            name='Invalid Badge',
            description='Badge with invalid extension',
            created_by=self.user,
            slug='invalid-badge',
            issuer=self.issuer,
            image='badge.png',
        ...
        extension_invalid = BadgeClassExtension.objects.create(
            badgeclass=badge_class_invalid,
            name='extensions:CompetencyExtension',
            original_json='invalid json string'
        )
