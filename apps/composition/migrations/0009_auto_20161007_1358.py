...
def migrate_duplicate_collection_badges(apps, schema_editor):
    ...
    for entry in LocalBadgeInstanceCollection.objects.all():
        if entry.instance_id:
            try:
                issuer_instance = BadgeInstance.objects.get(
                    slug=entry.instance.json.get("uid")
                )
                ...
                print(
                    (
                        "Migrating duplicate badge Collection.pk={}: LocalBadgeInstance.pk={} -> BadgeInstance.slug={}".format(
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(migrate_duplicate_collection_badges)]
