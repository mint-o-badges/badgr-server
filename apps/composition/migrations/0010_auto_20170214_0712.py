        ...
        if local_issuer.image:
            ...
            try:
                status_code, image = fetch_remote_file_to_storage(
                    remote_image_url, upload_to=local_issuer.image.field.upload_to
                )
            except IOError:
                image = None
        else:
...
def import_localbadgeclass_to_issuer(apps, schema_editor):
    ...
    for local_badgeclass in LocalBadgeClass.objects.all():
        issuer = Issuer.objects.get(
            source="legacy_local_issuer", source_url=local_badgeclass.issuer.identifier
        )
        ...
        if local_badgeclass.image:
            ...
            try:
                status_code, image = fetch_remote_file_to_storage(
                    remote_image_url, upload_to=local_badgeclass.image.field.upload_to
                )
            except IOError:
                image = None
        else:
