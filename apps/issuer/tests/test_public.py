...
from backpack.tests.utils import CURRENT_DIRECTORY, setup_basic_1_0, setup_resources
...
class PublicAPITests(SetupIssuerHelper, BadgrTestCase):
    ...
    def test_get_issuer_object(self):
        ...
        with self.assertNumQueries(0):
            response = self.client.get(
                "/public/issuers/{}".format(test_issuer.entity_id)
            )
    ...
    def test_get_badgeclass_image_with_redirect(self):
        ...
        with self.assertNumQueries(0):
            response = self.client.get(
                "/public/badges/{}/image".format(test_badgeclass.entity_id)
            )
    ...
    def test_get_badgeclass_image_wide(self):
        ...
        response = self.client.get(
            "/public/badges/{}".format(test_badgeclass.entity_id), **headers
        )
        # should have received an html stub with og meta tags
        self.assertTrue(response.get("content-type").startswith("text/html"))
        self.assertContains(
            response, "fmt=wide"
        )  # ensure the image is linked properly with the wide format

        with self.assertNumQueries(0):
            response = self.client.get(
                "/public/badges/{}/image?type=png&fmt=wide".format(
        ...
        response = self.client.get(
            response.url
        )  # Get the actual image URL from media storage
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_assertion_image_with_redirect(self):
        ...
        with self.assertNumQueries(0):
            response = self.client.get(
                "/public/assertions/{}/image".format(assertion.entity_id), follow=False
            )
            self.assertEqual(response.status_code, 302)
    ...
    def test_get_for_revoked_assertion_and_deleted_badge_class_returns_404(self):
        ...
        response = self.client.get(
            "/public/assertions/{}?action=download".format(assertion.entity_id)
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_scrapers_get_html_stub(self):
        ...
        BackpackCollectionBadgeInstance.objects.create(
            collection=test_collection, badgeinstance=assertion, badgeuser=test_user
        )  # add assertion to collection
        ...
        testcase_headers = [
            ...
            {
                "HTTP_USER_AGENT": "LinkedInBot/1.0 (compatible; Mozilla/5.0; Jakarta Commons-HttpClient/3.1 +http://www.linkedin.com)"  # noqa E501
            },
        ...
        for headers in testcase_headers:
            with self.assertNumQueries(0):
                response = self.client.get(
                    "/public/assertions/{}".format(assertion.entity_id), **headers
                )
                self.assertEqual(response.status_code, 200)
                ...
                png_image_url = "{}{}?type=png".format(
                    ...
                    reverse(
                        "badgeclass_image",
                        kwargs={"entity_id": assertion.cached_badgeclass.entity_id},
                    ),
                )
                self.assertContains(
                    response,
                    '<meta property="og:image" content="{}'.format(png_image_url),
                )
    ...
    def test_public_collection_json(self):
        ...
        response = self.client.get(
            "/public/collections/{}".format(test_collection.share_hash),
    ...
    def test_get_assertion_html_redirects_to_frontend(self):
        badgr_app = BadgrApp(
            cors="frontend.ui",
            is_default=True,
            signup_redirect="http://frontend.ui/signup",
            public_pages_redirect="http://frontend.ui/public",
        )
        badgr_app.save()

        badgr_app_two = BadgrApp(
            cors="stuff.com",
            is_default=False,
    ...
    def test_get_versioned_baked_images(self):
        ...
        response = self.client.get(
            "/public/assertions/{}/image".format(assertion.entity_id), follow=True
        )
        self.verify_baked_image_response(
            assertion, response, obi_version=UNVERSIONED_BAKED_VERSION
        )

        for obi_version in list(OBI_VERSION_CONTEXT_IRIS.keys()):
            response = self.client.get(
                "/public/assertions/{assertion}/baked?v={version}".format(
                    assertion=assertion.entity_id, version=obi_version
                ),
                follow=True,
            )
            ...
            if obi_version == UNVERSIONED_BAKED_VERSION:
                ...
                self.verify_baked_image_response(
                    assertion, response, obi_version=obi_version
                )
            else:
                self.verify_baked_image_response(
                    assertion,
    ...
    def test_cache_updated_on_issuer_update(self):
        ...
        test_badgeclass = self.setup_badgeclass(
            issuer=test_issuer, name=original_badgeclass_name
        )
        assertion = test_badgeclass.issue(recipient_id="new.recipient@email.test")

        response = self.client.get(
            "/public/assertions/{}?expand=badge".format(assertion.entity_id),
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.get("badge", {}).get("name", None), new_badgeclass_name
        )
...
class PendingAssertionsPublicAPITests(SetupIssuerHelper, BadgrTestCase):
    ...
    def test_pending_assertion_returns_404(self):
        ...
        with mock.patch(
            ...
            self.client.post("/v2/backpack/import", post_input, format="json")
...
class OEmbedTests(SetupIssuerHelper, BadgrTestCase):
    ...
    def test_get_oembed_json(self):
        ...
        # with self.assertNumQueries(0):
        response = self.client.get(
            "/public/oembed?format=json&url={}".format(
                urllib.parse.quote(assertion.jsonld_id)
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_endpoint_handles_malformed_urls(self):
        response = self.client.get(
            "/public/oembed?format=json&url={}".format(
                urllib.parse.quote("ralph the dog")
            )
        )
        self.assertEqual(response.status_code, 404)
...
class PublicReverificationTests(SetupIssuerHelper, BadgrTestCase, Ob2Generators):
    ...
    def test_can_reverify_basic(self, mock_verify):
        ...
        assertion_ob2 = self.generate_assertion_ob2(
            source_url="https://example.com/assertion/1"
        )
        ...
        badgeinstance_image = BadgeInstance.objects.image_from_ob2(
            badgeclass_image, assertion_ob2
        )

        issuer, _ = Issuer.objects.get_or_create_from_ob2(
            issuer_ob2, image=issuer_image
        )
        badgeclass, _ = BadgeClass.objects.get_or_create_from_ob2(
            issuer, badgeclass_ob2, image=badgeclass_image
        )
        ...
        with mock.patch(
            ...
            mock_verify.return_value = {
                "report": self.generate_ob2_report(
                    validationSubject=assertion_ob2["id"]
                ),
                "graph": [assertion_ob2, badgeclass_ob2, issuer_ob2],
                "input": self.generate_ob2_input(
                    input_type="url", value=assertion_ob2["id"]
                ),
            }

            # openbadges.verify response (Not Revoked)
            verify_response = self.client.post(
                "/public/verify", data={"entity_id": assertion.entity_id}
            )
            ...
            self.assertEqual(
                revoked_response.data["result"][0]["revocationReason"],
                revocation_reason,
            )
            ...
            second_revoked_response = self.client.post(
                "/public/verify", data={"entity_id": assertion.entity_id}
            )
            # still revoked
            ...
            self.assertEqual(
                second_revoked_response.data["result"][0]["revocationReason"],
                revocation_reason,
            )
            # badge instance is revoked, revocation_reason has not changed
            self.assertTrue(BadgeInstance.objects.last().revoked)
            ...
            third_revoked_response = self.client.post(
                "/public/verify", data={"entity_id": assertion.entity_id}
            )
            # still revoked
            ...
            self.assertEqual(
                third_revoked_response.data["result"][0]["revocationReason"],
                revocation_reason,
            )
            # badge instance is revoked, revocation_reason has not changed
            self.assertTrue(BadgeInstance.objects.last().revoked)
