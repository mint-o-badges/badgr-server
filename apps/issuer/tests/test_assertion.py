...
from issuer.models import BadgeInstance, EmailBlacklist, Issuer, IssuerStaff
from issuer.utils import parse_original_datetime
from mainsite.tests import (
    BadgrTestCase,
    SetupIssuerHelper,
    SetupOAuth2ApplicationHelper,
)
...
class AssertionTests(SetupIssuerHelper, BadgrTestCase):
    ...
    def test_assertion_pagination(self):
        ...
        response = self.client.get(
            "/v1/issuer/issuers/{issuer}/badges/{badgeclass}/assertions?num={per_page}".format(
    ...
    def test_put_rebakes_assertion(self):
        ...
        updated_data = dict(expires=v1_backdate.isoformat())

        response = self.client.put(
            "/v1/issuer/issuers/{issuer}/badges/{badge}/assertions/{assertion}".format(
        ...
        self.assertEqual(response.status_code, 200)
        updated_assertion = BadgeInstance.objects.get(
            entity_id=test_assertion.entity_id
        )
        ...
        updated_data = dict(issuedOn=v2_backdate.isoformat())

        response = self.client.put(
            "/v2/assertions/{assertion}".format(assertion=test_assertion.entity_id),
            updated_data,
        )
        self.assertEqual(response.status_code, 200)
        updated_assertion = BadgeInstance.objects.get(
    ...
    def test_updating_badgeclass_image_rebakes_assertions(self):
        test_user = self.setup_user(authenticate=True)
        test_issuer = self.setup_issuer(owner=test_user)
        test_badgeclass_v1 = self.setup_badgeclass(
            name="for v1 test", issuer=test_issuer
        )
        test_badgeclass_v2 = self.setup_badgeclass(
            name="for v2 test", issuer=test_issuer
        )
        ...
        with open(self.get_test_svg_image_path(), "rb") as image_update:
            ...
            response = self.client.put(
                "/v1/issuer/issuers/{issuer}/badges/{badgeclass}".format(
            ...
            self.assertEqual(response.status_code, 200)
            sleep(2)
            updated_assertion_v1 = BadgeInstance.objects.get(
                entity_id=test_assertion_v1.entity_id
            )
            ...
            self.assertGreater(
                updated_assertion_v1.updated_at, test_assertion_v1.updated_at
            )
        ...
        with open(self.get_test_svg_image_path(), "rb") as image_update:
            ...
            self.assertEqual(response.status_code, 200)
            sleep(2)
            updated_assertion_v2 = BadgeInstance.objects.get(
                entity_id=test_assertion_v2.entity_id
            )
            ...
            self.assertGreater(
                updated_assertion_v2.updated_at, test_assertion_v2.updated_at
            )
            self.assertEqual(updated_assertion_v2.image.name.split(".").pop(), "svg")
            response = self.client.get(
                "/v2/assertions/{}".format(updated_assertion_v2.entity_id)
            )
            self.assertEqual(response.status_code, 200)
            result = response.data["result"][0]
            self.assertIn(
                "{}/image".format(updated_assertion_v2.entity_id), result["image"]
            )  # canonical image url

            # test batching works in task
            updated_assertion_v2_2 = BadgeInstance.objects.get(
                entity_id=test_assertion_v2_2.entity_id
            )
            ...
            self.assertGreater(
                updated_assertion_v2_2.updated_at, test_assertion_v2_2.updated_at
            )
    ...
    def test_updating_badgeclass_non_image_does_not_rebake_assertions(self):
        ...
        response = self.client.put(
            ...
            dict(name="some name update 3", description="some description 3"),
        )
        self.assertEqual(response.status_code, 200)
        sleep(2)
        updated_assertion_v2 = BadgeInstance.objects.get(
    ...
    def test_can_update_assertion(self):
        ...
        response = self.client.post(
            "/v1/issuer/issuers/{issuer}/badges/{badge}/assertions".format(
                issuer=test_issuer.entity_id, badge=test_badgeclass.entity_id
            ),
            assertion_data,
        )
        ...
        response = self.client.put(
            "/v2/assertions/{assertion}".format(
                assertion=original_assertion.get("slug")
            ),
            json.dumps(v2_assertion_data),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(
            v2_assertion["evidence"][0]["narrative"],
            v2_assertion_data["evidence"][0]["narrative"],
        )
        ...
        self.assertEqual(
            image_data.get("evidence", {})[0].get("narrative"),
            v2_assertion_data["evidence"][0]["narrative"],
        )

    def test_can_update_assertion_issuer(self):
        test_user = self.setup_user(authenticate=True)
        email_two = CachedEmailAddress.objects.create(
            email="testemail2@example.com", verified=True, user=test_user
        )
        test_issuer = self.setup_issuer(owner=test_user)
        test_badgeclass = self.setup_badgeclass(issuer=test_issuer)
    ...
    def test_can_issue_assertion_with_expiration(self):
        ...
        self.assertEqual(
            dateutil.parser.parse(assertion_json.get("expires")), expiration
        )
        ...
        response = self.client.get(
            "/v2/assertions/{assertion}".format(assertion=assertion_json.get("slug"))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_can_issue_badge_if_authenticated(self):
        ...
        assertion = {"email": "test@example.com", "create_notification": False}
        response = self.client.post(
            "/v1/issuer/issuers/{issuer}/badges/{badge}/assertions".format(
                issuer=test_issuer.entity_id, badge=test_badgeclass.entity_id
            ),
    ...
    def test_can_issue_badge_by_class_name_success(self):
        ...
        response = self.client.post(
            "/v2/issuers/{issuer}/assertions".format(issuer=test_issuer.entity_id),
            assertion,
            format="json",
        )
        self.assertEqual(response.status_code, 201)
    ...
    def test_cannot_issue_badge_to_invalid_email_error(self):
        ...
        assertion = {
            "recipient": {"identity": "example.com", "type": "email"},
            "badgeclassName": badgeclass_name,
        }
    ...
    def test_badge_recipient_notified_by_email_before_blacklisting_but_not_after(self):
        ...
        self.assertEqual(response.status_code, 201)
        instance = BadgeInstance.objects.get(
    ...
    def test_issue_badge_with_ob1_evidence(self):
        ...
        self.assertEqual(
            response.data.get("evidence_items"),
            [
                {
    ...
    def test_issue_badge_with_ob2_multiple_evidence(self):
        ...
        for i in range(0, len(evidence_items)):
            self.assertEqual(
                fetched_evidence_items[i].get("url"), evidence_items[i].get("url")
            )
            self.assertEqual(
                fetched_evidence_items[i].get("narrative"),
                evidence_items[i].get("narrative"),
            )

        # ob1.0 evidence url also present
        self.assertIsNotNone(assertion.get("json"))
        assertion_public_url = OriginSetting.HTTP + reverse(
            "badgeinstance_json", kwargs={"entity_id": assertion_slug}
        )
    ...
    def test_v2_issue_with_evidence(self):
        ...
        response = self.client.post(
            "/v2/badgeclasses/{badge}/assertions".format(
                badge=test_badgeclass.entity_id
            ),
            assertion_args,
            format="json",
        ...
        v2_json = self.client.get(
            "/public/assertions/{}?v=2_0".format(assertion_slug), format="json"
        ).data
        ...
        for i in range(0, len(evidence_items)):
            self.assertEqual(
                v2_json["evidence"][i].get("id"), evidence_items[i].get("url")
            )
            self.assertEqual(
                v2_json["evidence"][i].get("narrative"),
                evidence_items[i].get("narrative"),
            )
            self.assertEqual(
                fetched_evidence_items[i].get("evidence_url"),
                evidence_items[i].get("url"),
            )
            self.assertEqual(
                fetched_evidence_items[i].get("narrative"),
                evidence_items[i].get("narrative"),
    ...
    def test_resized_png_image_baked_properly(self):
        ...
        for chunk in reader.chunks():
            ...
            elif chunk[0] == b"iTXt" and chunk[1].startswith(
                b"openbadges\x00\x00\x00\x00\x00"
            ):
    ...
    def test_authenticated_editor_can_issue_badge(self):
        ...
        IssuerStaff.objects.create(
            issuer=test_issuer, role=IssuerStaff.ROLE_EDITOR, user=editor_user
        )
    ...
    def test_authenticated_nonowner_user_cant_issue(self):
        ...
        assertion = {"email": "test2@example.com"}
        response = self.client.post(
            "/v1/issuer/issuers/{issuer}/badges/{badge}/assertions".format(
                issuer=test_issuer.entity_id,
                badge=test_badgeclass.entity_id,
    ...
    def test_issuer_instance_list_assertions(self):
        ...
        response = self.client.get(
            "/v1/issuer/issuers/{issuer}/assertions".format(
                issuer=test_issuer.entity_id,
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_issuer_instance_list_assertions_with_expired(self):
        ...
        test_issuer = self.setup_issuer(owner=test_user)
        test_badgeclass = self.setup_badgeclass(issuer=test_issuer)
        expired_assertion = test_badgeclass.issue(
            recipient_id="new.recipient@email.test"
        )
        expired_assertion.expires_at = datetime.datetime.now() - datetime.timedelta(
            days=1
        )
        ...
        response = self.client.get(
            "/v2/issuers/{issuer}/assertions".format(
                issuer=test_issuer.entity_id,
            )
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            "/v2/issuers/{issuer}/assertions?include_expired=1".format(
                issuer=test_issuer.entity_id,
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_issuer_instance_list_assertions_with_revoked(self):
        ...
        test_badgeclass = self.setup_badgeclass(issuer=test_issuer)
        test_badgeclass.issue(recipient_id="new.recipient@email.test")
        revoked_assertion = test_badgeclass.issue(
            recipient_id="second.recipient@email.test"
        )
        revoked_assertion.revoked = True
        revoked_assertion.save()

        response = self.client.get(
            "/v2/issuers/{issuer}/assertions?include_revoked=1".format(
                issuer=test_issuer.entity_id,
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_issuer_instance_list_assertions_with_revoked_and_expired(self):
        ...
        response = self.client.get(
            "/v2/issuers/{issuer}/assertions?include_revoked=1&include_expired=1".format(
                issuer=test_issuer.entity_id,
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_issuer_instance_list_assertions_with_id(self):
        ...
        response = self.client.get(
            "/v1/issuer/issuers/{issuer}/assertions?recipient=new.recipient@email.test".format(
                issuer=test_issuer.entity_id,
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_can_revoke_assertion(self):
        ...
        response = self.client.get(
            "/public/assertions/{assertion}.json".format(
                assertion=test_assertion.entity_id
            )
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertDictContainsSubset(
            dict(revocationReason=revocation_reason, revoked=True), assertion_obo
        )
    ...
    def test_can_revoke_assertion_bulk(self):
        ...
        response = self.client.post(
            reverse("v2_api_assertion_revoke"), data=revocation_data, format="json"
        )
        self.assertEqual(response.status_code, 200)
        ...
        assertion_obo = json.loads(response.content)
        self.assertDictContainsSubset(
            dict(revocationReason=revocation_data[0]["revocationReason"], revoked=True),
            assertion_obo,
        )
    ...
    def test_issue_svg_badge(self):
        ...
        with open(self.get_test_svg_image_path(), "rb") as svg_badge_image:
            response = self.client.post(
                "/v1/issuer/issuers/{issuer}/badges".format(
                    issuer=test_issuer.entity_id,
                ),
                {
                    "name": "svg badge",
        ...
        response = self.client.post(
            "/v1/issuer/issuers/{issuer}/badges/{badge}/assertions".format(
                issuer=test_issuer.entity_id, badge=badgeclass_slug
            ),
            assertion,
        )
        self.assertEqual(response.status_code, 201)
        ...
        response = self.client.get(
            "/v1/issuer/issuers/{issuer}/badges/{badge}/assertions/{assertion}".format(
                issuer=test_issuer.entity_id, badge=badgeclass_slug, assertion=slug
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_new_assertion_updates_cached_user_badgeclasses(self):
        ...
        original_recipient_count = test_badgeclass.badgeinstances.filter(
            revoked=False
        ).count()
        ...
        response.data
        self.assertEqual(
            test_badgeclass.badgeinstances.filter(revoked=False).count(),
    ...
    def test_batch_assertions_throws_400(self):
        ...
        test_issuer = self.setup_issuer(owner=test_user)
        test_badgeclass = self.setup_badgeclass(issuer=test_issuer)
        invalid_batch_assertion_props = [{"recipient": {"identity": "foo@bar.com"}}]
        response = self.client.post(
            "/v2/badgeclasses/{badge}/issue".format(badge=test_badgeclass.entity_id),
    ...
    def test_batch_assertions_with_invalid_issuedon(self):
        ...
        invalid_batch_assertion_props = {
            "assertions": [
                {"recipient": {"identity": "foo@bar.com", "type": "email"}},
                {
                    "recipient": {"identity": "bar@baz.com", "type": "email"},
    ...
    def test_issue_assertion_with_unacceptable_issuedOn(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.data["fieldErrors"]["issuedOn"][0],
            "Only issuedOn dates in the past are acceptable.",
        )

        assertion_data["issuedOn"] = (
            "1492-01-01T13:00:00Z"  # A time prior to introduction of the Gregorian calendar.
        )
        response = self.client.post(
            "/v2/badgeclasses/{badge}/assertions".format(
                badge=test_badgeclass.entity_id
    ...
    def test_batch_assertions_with_evidence(self):
        ...
        for i in range(0, len(returned_assertions)):
            ...
            self.assertListOfDictsContainsSubset(
                expected.get("evidence"), returned_assertions[i].get("evidence")
            )
        ...
        response = self.client.get(
            "/public/assertions/{assertion}.json?v=2_0".format(
        ...
        for i in range(0, len(expected)):
            ...
            self.assertEqual(
                evidence[i].get("narrative", None), expected[i].get("narrative", None)
            )
    ...
    def test_get_share_url(self):
        ...
        test_badgeclass = self.setup_badgeclass(issuer=test_issuer)
        test_assertion = test_badgeclass.issue(recipient_id="new.recipient@email.test")
        test_assertion2 = test_badgeclass.issue(
            recipient_id="+15035555555", recipient_type="telephone"
        )
        test_assertion3 = test_badgeclass.issue(
            recipient_id="test.example.com/foo?bar=1", recipient_type="url"
        )
        ...
        self.assertEqual(
            test_assertion.jsonld_id + "?identity__email=new.recipient%40email.test",
            url,
        )
        url = test_assertion2.get_share_url(include_identifier=True)
        self.assertEqual(
            test_assertion2.jsonld_id + "?identity__telephone=%2B15035555555", url
        )
        url = test_assertion3.get_share_url(include_identifier=True)
        self.assertEqual(
            test_assertion3.jsonld_id + "?identity__url=test.example.com/foo%3Fbar%3D1",
            url,
        )
...
class V2ApiAssertionTests(SetupIssuerHelper, BadgrTestCase):
    def test_v2_issue_by_badgeclassOpenBadgeId(self):
        ...
        new_assertion_props = {
            "recipient": {"identity": "test3@example.com"},
            "badgeclassOpenBadgeId": test_badgeclass.jsonld_id,
        }
        response = self.client.post(
            "/v2/issuers/{issuer}/assertions".format(issuer=test_issuer.entity_id),
        ...
        result = response.data["result"][0]
        self.assertIn(
            "{}/image".format(result["entityId"]), result["image"]
        )  # canonical image url
    ...
    def test_v2_issue_uppercase_email(self):
        ...
        self.assertEqual(
            recipient_id,
            new_assertion_props["recipient"]["identity"].lower(),
            "Reported recipient ID is lowercase",
        )
        assertion_from_db = BadgeInstance.objects.get(
            entity_id=assertion_data["entityId"]
        )
        self.assertEqual(
            assertion_from_db.recipient_identifier,
            new_assertion_props["recipient"]["identity"].lower(),
    ...
    def test_v2_issue_uppercase_url(self):
        ...
        self.assertEqual(
            recipient_id,
            "https://example.com/UPPERCASEPATH",
        ...
        new_assertion_props["recipient"][
            "identity"
        ] = "NOTAURL/NOTAVALIDONE/butlookslikeone?sorta=True"
        response = self.client.post(
            "/v2/issuers/{issuer}/assertions".format(issuer=test_issuer.entity_id),
            new_assertion_props,
            format="json",
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.json()["fieldErrors"]["recipient"]["non_field_errors"][0],
            "Enter a valid URL.",
        )
    ...
    def test_v2_issue_entity_id_in_path(self):
        ...
        new_assertion_props = {"recipient": {"identity": "test3@example.com"}}
        response = self.client.post(
            "/v2/badgeclasses/{badgeclass}/assertions".format(
                badgeclass=test_badgeclass.entity_id
            ),
            new_assertion_props,
            format="json",
        ...
        response = self.client.post(
            "/v2/badgeclasses/{badgeclass}/assertions".format(
                badgeclass=other_badgeclass.entity_id
            ),
            new_assertion_props,
            format="json",
        )
    ...
    def test_can_revoke_assertion(self):
        ...
        revocation_reason = (
            "I take it all back. I don't mean what I said when I was hungry."
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data["result"][0]["revocationReason"], revocation_reason
        )
...
class AssertionsChangedApplicationTests(
    SetupOAuth2ApplicationHelper, SetupIssuerHelper, BadgrTestCase
):
    def test_application_can_get_changed_assertions(self):
        ...
        IssuerStaff.objects.create(
            issuer=test_issuer, role=IssuerStaff.ROLE_STAFF, user=application_user
        )
        ...
        self.assertEqual(
            response.status_code, 200, "Can get a token for the issuer user"
        )
        ...
        self.assertEqual(
            response.status_code, 200, "Can get a token for the application user"
        )

        test_badgeclass.issue(recipient_id="test@example.com")

        self.client.credentials(
            HTTP_AUTHORIZATION="Bearer " + response.json()["access_token"]
        )
        ...
        response = self.client.get(
            "/v2/assertions/changed?since={}".format(quote_plus(timestamp))
        )
        self.assertEqual(response.status_code, 200)
...
class AssertionWithUserTests(SetupIssuerHelper, BadgrTestCase):
    def setUp(self):
        ...
        self.issuer = self.setup_issuer(
            owner=self.setup_user(email="staff@example.com")
        )
    ...
    def test_verification_change_owns_badge(self):
        recipient = self.setup_user(
            email="recipient@example.com", authenticate=False, verified=False
        )
        badgeclass = self.setup_badgeclass(issuer=self.issuer)
        ...
        my_id = UserRecipientIdentifier.objects.create(
        ...
        badgeclass.issue(
            ...
            recipient_type=UserRecipientIdentifier.IDENTIFIER_TYPE_TELEPHONE,
        )
...
class AllowDuplicatesAPITests(SetupIssuerHelper, BadgrTestCase):
    def test_single_award_allow_duplicates(self):
        ...
        response = self.client.post(
            "/v2/badgeclasses/{}/assertions".format(test_badgeclass.entity_id),
            new_assertion_props,
            format="json",
        )
        self.assertEqual(response.status_code, 400)
        ...
        self.assertEqual(
            ...
            201,
            "Assertion should be allowed if existing award is revoked",
        )
    ...
    def test_single_award_allow_duplicates_against_not_expired(self):
        ...
        self.assertEqual(
            response.status_code,
            400,
            "The badge should not award, given a unexpired existing award",
        )
    ...
    def test_single_award_allow_duplicates_against_expired(self):
        ...
        self.assertEqual(
            response.status_code,
            201,
            "The badge should award, given an expired prior award.",
        )
    ...
    def test_badgeclass_and_issuer_not_in_assertion_cache_record(self):
        ...
        )  # call the foreign key attribute to ensure the related object is cached
