...
from mainsite.tests import BadgeUser, BadgrTestCase, SetupIssuerHelper
...
class AssertionsChangedSinceTests(SetupIssuerHelper, BadgrTestCase):
    ...
    def test_with_two_apps(self):
        # Application A
        client_a = BadgeUser.objects.create(
        ...
        # Application B
        client_b = BadgeUser.objects.create(
    ...
    def test_application_can_fetch_changed_assertions(self):
        ...
        clientAppUser = self.setup_user(
            email="clientApp@example.com", token_scope="r:assertions"
        )
        ...
        unrelated_app = Application.objects.create(
            client_id="clientApp-authcode-2",
            client_secret="testsecret",
            authorization_grant_type="authorization-code",
            user=None,
        )
        AccessTokenProxy.objects.create(
            user=unrelated_recipient,
            scope="rw:issuer r:profile r:backpack",
            expires=timezone.now() + timedelta(hours=1),
            token="abc3",
            application=unrelated_app,
        )
    ...
    def assertions_in_expected_order_through_pagination(self):
        ...
        response = self.client.get(
            reverse("v2_api_assertions_changed_list")
            + "?num=5&since={}".format(cut_time)
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            len(response.data["result"]), 5
        )  # There are the expected number of results in a page
        self.assertEqual(
            response.data["result"][0]["entityId"], assertions[5].entity_id
        )
        self.assertEqual(
            response.data["result"][4]["entityId"], assertions[9].entity_id
        )
        ...
        self.assertEqual(
            response.data["result"][4]["entityId"], assertions[14].entity_id
        )
...
class AssertionFetching(SetupIssuerHelper, BadgrTestCase):
    def test_can_paginate_fetch_assertions_by_recipient(self):
        ...
        url = "{url}?num=2&recipient={email3}".format(
            url=reverse(
                "v2_api_badgeclass_assertion_list",
    ...
    def test_can_fetch_assertions_by_recipient_ids(self):
        ...
        url = "{url}".format(
            url=reverse(
                "v2_api_issuer_assertion_list", kwargs={"entity_id": issuer.entity_id}
            ),
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_can_fetch_assertions_by_url_based_recipient_ids(self):
        ...
        UserRecipientIdentifier.objects.create(
            identifier="http://example.com/notme", user=u1
        )
        ...
        url = "{url}?recipient={url_recipient}".format(
            url=reverse(
                "v2_api_badgeclass_assertion_list", kwargs={"entity_id": b.entity_id}
            ),
...
class AssertionPosting(SetupIssuerHelper, BadgrTestCase):
    def test_can_post_assertion_to_v2_api_badgeclass_assertion_list_with_issuer_specified(
        self,
    ):
        u1 = self.setup_user(authenticate=True, email="hey@example.com")
        ...
        response = self.client.post(
            ...
            data={
                "issuer": i.entity_id,
                "recipient": {"identity": u1.email, "hashed": True, "type": "email"},
            },
            format="json",
        )
        ...
        response2 = self.client.post(
            ...
            data={
                "issuer": i2.entity_id,
                "recipient": {"identity": u2.email, "hashed": True, "type": "email"},
            },
            format="json",
        )
        ...
        response3 = self.client.post(
            url,
            data={"recipient": {"identity": u3.email, "hashed": True, "type": "email"}},
            format="json",
        )
