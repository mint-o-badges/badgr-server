...
class PublicAPITests(SetupOAuth2ApplicationHelper, SetupIssuerHelper, BadgrTestCase):
    ...
    def test_can_get_issuer_scoped_token(self):
        ...
        application_user = self.setup_user(authenticate=False)
        application = self.setup_oauth2_application(
            user=application_user, allowed_scopes="rw:issuer rw:issuer:*"
        )
    ...
    def test_can_get_batch_issuer_tokens(self):
        ...
        application_user = self.setup_user(
            email="service@email.test", authenticate=False
        )
        application = self.setup_oauth2_application(
            user=application_user, allowed_scopes="rw:issuer"
        )
        ...
        auth_headers = {
            "Authorization": "{type} {token}".format(
                type=result.get("token_type"), token=result.get("access_token")
            )
        }
        ...
        issuers = [
            self.setup_issuer(owner=badgr_user, name="issuer #{}".format(i))
            for i in range(1, 4)
        ]
        ...
        response = self.client.post(
            reverse("v2_api_tokens_list"),
            data=dict(issuers=issuer_ids),
            format="json",
            **auth_headers,
        )
        self.assertEqual(response.status_code, 200)
        ...
        access_tokens = [
            AccessTokenProxy.objects.get(token=t) for t in list(issuer_tokens.values())
        ]
        ...
        for issuer_id, issuer_token in list(issuer_tokens.items()):
            response = self.client.get(
                reverse("v2_api_issuer_detail", kwargs=dict(entity_id=issuer_id)),
                format="json",
