...
class BadgeClassTests(SetupIssuerHelper, BadgrTestCase):
    ...
    def _create_badgeclass_for_issuer_authenticated(self, image_path, **kwargs):
        with open(image_path, "rb") as badge_image:
            ...
            image_str = self._base64_data_uri_encode(
                badge_image, kwargs.get("image_mimetype", "image/png")
            )
            ...
            with self.assertNumQueries(
                1
            ):  # the v1 badgeclass GET API now relies on a query for recipient_count
                response = self.client.get(
                    "/v1/issuer/issuers/{issuer}/badges/{badgeclass}".format(
                        issuer=test_issuer.entity_id, badgeclass=new_badgeclass_slug
                    )
                )
    ...
    def test_staff_cannot_create_badgeclass(self):
        with open(self.get_test_image_path(), "rb") as badge_image:
            ...
            IssuerStaff.objects.create(
                issuer=test_issuer, user=test_user, role=IssuerStaff.ROLE_STAFF
            )
            self.issuer = test_issuer
            response = self.client.post(
                "/v1/issuer/issuers/{slug}/badges".format(slug=test_issuer.entity_id),
    ...
    def test_v2_post_put_badgeclasses_permissions(self):
        ...
        response = self.client.post(
            "/v2/badgeclasses", data=badgeclass_data, format="json"
        )
        self.assertEqual(response.status_code, 400)

        staff_record = IssuerStaff.objects.create(
            issuer=test_issuer, user=test_user, role=IssuerStaff.ROLE_STAFF
        )
        response = self.client.post(
            "/v2/badgeclasses", data=badgeclass_data, format="json"
        ...
        response = self.client.put(
            "/v2/badgeclasses/{}".format(entity_id), data=badgeclass_data, format="json"
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_v2_badgeclasses_reasonable_404_error(self):
        test_owner = self.setup_user(authenticate=False)
        self.setup_issuer(owner=test_owner)
        self.setup_user(authenticate=True, token_scope="rw:issuer")
    ...
    def test_v2_badgeclasses_can_paginate(self):
        ...
        test_user = self.setup_user(authenticate=True)
        test_issuer = self.setup_issuer(owner=test_user)
        list(self.setup_badgeclasses(issuer=test_issuer, how_many=NUM_BADGE_CLASSES))

        test_user2 = self.setup_user(authenticate=True)
        test_issuer2 = self.setup_issuer(owner=test_user2)
        test_badgeclass2 = list(
            self.setup_badgeclasses(issuer=test_issuer2, how_many=NUM_BADGE_CLASSES)
        )
    ...
    def test_badgeclass_with_expires_in_days_v1(self):
        ...
        v1_data.update(
            dict(
                expires=dict(amount=10, duration="days"),
            )
        )
        response = self.client.post(
            "/v1/issuer/issuers/{issuer}/badges".format(issuer=test_issuer.entity_id),
            data=v1_data,
            format="json",
        )
        self.assertEqual(response.status_code, 201)
        ...
        def _update_badgeclass(data):
            return self.client.put(
                "/v1/issuer/issuers/{issuer}/badges/{badge}".format(
                    issuer=test_issuer.entity_id, badge=badgeclass_entity_id
                ),
                data=data,
                format="json",
            )
    ...
    def test_badgeclass_with_expires_in_days_v2(self):
        ...
        v2_data.update(dict(expires=dict(amount=10, duration="days")))
        ...
        def _update_badgeclass(data):
            return self.client.put(
                "/v2/badgeclasses/{badge}".format(badge=new_badgeclass.get("entityId")),
                data=data,
                format="json",
            )
        ...
        for good_data in good_expires_values:
            ...
            self.assertDictEqual(
                updated_badgeclass.get("expires"), v2_data.get("expires")
            )
    ...
    def test_badgeclass_relative_expire_date_generation(self):
        ...
        date = badgeclass.generate_expires_at(
            issued_on=timezone.datetime(
                year=2018, month=8, day=29, hour=12, tzinfo=timezone.utc
            )
        )
        ...
        date = badgeclass.generate_expires_at(
            issued_on=timezone.datetime(
                year=2020, month=2, day=29, hour=12, tzinfo=timezone.utc
            )
        )
    ...
    def test_can_create_badgeclass_with_svg(self):
        self._create_badgeclass_for_issuer_authenticated(
            self.get_test_svg_image_path(), image_mimetype="image/svg+xml"
        )

    def test_can_get_png_preview_for_svg_badgeclass(self):
        badgeclass_data = self._create_badgeclass_for_issuer_authenticated(
            self.get_test_svg_image_path(), image_mimetype="image/svg+xml"
        )

        response = self.client.get(
            "/public/badges/{}/image?type=png".format(badgeclass_data.get("slug"))
        )
    ...
    def test_create_badgeclass_scrubs_svg(self):
        with open(
            self.get_testfiles_path("hacked-svg-with-embedded-script-tags.svg"), "rb"
    ...
    def test_when_creating_badgeclass_with_criteriatext_criteraurl_is_returned(self):
        ...
        with open(self.get_test_image_path(), "rb") as badge_image:
            ...
            expected_criterial_url = OriginSetting.HTTP + reverse(
                "badgeclass_criteria", kwargs={"entity_id": new_badgeclass_slug}
            )
            self.assertEqual(
                response.data.get("json").get("criteria"), expected_criterial_url
            )
    ...
    def test_can_get_badgeclass_list_if_authenticated(self):
        ...
        test_badgeclasses = list(
            self.setup_badgeclasses(issuer=test_issuer, how_many=3)
        )
    ...
    def test_cannot_get_badgeclass_list_if_unauthenticated(self):
        ...
        test_user = self.setup_user(authenticate=False)
        test_issuer = self.setup_issuer(owner=test_user)
        list(self.setup_badgeclasses(issuer=test_issuer))
    ...
    def test_can_delete_unissued_badgeclass(self):
        ...
        self.assertFalse(
            BadgeClass.objects.filter(entity_id=test_badgeclass.entity_id).exists()
        )
    ...
    def test_can_create_badgeclass_with_valid_markdown(self):
        with open(self.get_test_image_path(), "rb") as badge_image:
            ...
            badgeclass_props["criteria"] = (
                "This is *valid* markdown <p>mixed with raw</p> "
                '<script>document.write("and abusive html")</script>'
            )
            response = self.client.post(
                "/v1/issuer/issuers/{slug}/badges".format(slug=test_issuer.entity_id),
                badgeclass_props,
            ...
            self.assertEqual(
                new_badgeclass.get("criteria_text", None),
                'This is *valid* markdown mixed with raw document.write("and abusive html")',
            )
    ...
    def test_can_create_badgeclass_with_alignment(self):
        with open(self.get_test_image_path(), "rb") as badge_image:
            ...
            badgeclass_props["alignments"] = [
                {"targetName": "Align1", "targetUrl": "http://examp.e.org/frmwrk/1"},
                {"targetName": "Align2", "targetUrl": "http://examp.e.org/frmwrk/2"},
            ]
            ...
            self.assertEqual(
                new_badgeclass["alignments"][0]["targetName"],
                badgeclass_props["alignments"][0]["targetName"],
            )

            # verify that public page renders markdown as html
            response = self.client.get(
                "/public/badges/{}?v=2_0".format(new_badgeclass.get("entityId"))
            )
            self.assertIn("alignment", list(response.data.keys()))
            self.assertEqual(len(response.data["alignment"]), 2)
            self.assertEqual(
                response.data["alignment"][0]["targetName"],
                badgeclass_props["alignments"][0]["targetName"],
            )
    ...
    def test_new_badgeclass_updates_cached_issuer(self):
        ...
        with open(self.get_test_image_path(), "rb") as badge_image:
            ...
            self.assertEqual(
                len(list(test_user.cached_badgeclasses())), number_of_badgeclasses + 1
            )
    ...
    def test_issuer_edits_reflected_in_badgeclass(self):
        ...
        response = self.client.put(
            "/v2/issuers/{}".format(test_issuer.entity_id), data=issuer_data
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_v2_badgeclass_put_image_data_uri_resized_from_450_to_400(self):
        ...
        with open(self.get_testfiles_path("450x450.png"), "rb") as new_badge_image:
            put_response = self.client.put(
                ...
                dict(
                    badgeclass_props,
                    image=self._base64_data_uri_encode(new_badge_image, "image/png"),
                ),
            )
    ...
    def test_badgeclass_image_url_is_canonical(self):
        ...
        response = self.client.post(
            "/v2/badgeclasses", data=example_badgeclass_props, format="json"
        )
        self.assertEqual(response.status_code, 201)
    ...
    def test_badgeclass_put_image_non_data_uri(self):
        ...
        put_response = self.client.put(
            "/v1/issuer/issuers/{issuer}/badges/{badge}".format(
                issuer=test_issuer.entity_id, badge=slug
            ),
    ...
    def test_can_create_and_update_badgeclass_with_alignments_v1(self):
        ...
        new_badgeclass = self._create_badgeclass_for_issuer_authenticated(
            self.get_test_image_path(), alignment=alignments
        )
        ...
        new_badgeclass_url = "/v1/issuer/issuers/{issuer}/badges/{badgeclass}".format(
            issuer=self.issuer.entity_id, badgeclass=new_badgeclass["slug"]
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            updated_badgeclass.get("alignment", None), reordered_alignments
        )
    ...
    def test_can_create_and_update_badgeclass_with_alignments_v2(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            updated_badgeclass.get("alignments", None), reordered_alignments
        )
        self.assertEqual(
            updated_badgeclass.get("description", None), "refreshed description"
        )
    ...
    def test_can_create_and_update_badgeclass_with_tags_v1(self):
        ...
        new_badgeclass = self._create_badgeclass_for_issuer_authenticated(
            self.get_test_image_path(), tags=tags
        )
    ...
    def test_can_update_badgeclass_with_extensions(self):
        ...
        response = self.client.put(
            "/v2/badgeclasses/{badge}".format(badge=badgeclass.get("entityId")),
            data=badgeclass,
            format="json",
        )
        self.assertEqual(response.status_code, 200)
    ...
    def verify_badgeclass_extensions(self, badgeclass, example_extensions):
        ...
        response = self.client.get(
            "/public/badges/{badge}".format(badge=badgeclass.get("entityId"))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_null_description_not_serialized(self):
        ...
        response = self.client.get(
            "/v1/issuer/issuers/{issuer}/badges/{badgeclass}".format(
                issuer=test_issuer.entity_id, badgeclass=test_badgeclass.entity_id
            )
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            "/v2/badgeclasses/{badgeclass}".format(badgeclass=test_badgeclass.entity_id)
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.get("result", [])[0].get("description", None), ""
        )

        response = self.client.get(
            "/public/badges/{badgeclass}".format(badgeclass=test_badgeclass.entity_id)
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data.get("description", None), "")
    ...
    def test_updating_issuer_cache(self):
        ...
        test_badgeclass = self.setup_badgeclass(
            issuer=test_issuer, name="Badge 1", description="test"
        )
        ...
        response = self.client.put(
            "/v1/issuer/issuers/{}".format(test_issuer.entity_id), updated_issuer_props
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            "/public/assertions/{}.json?expand=badge&expand=badge.issuer".format(
                assertion_slug
            )
        )
    ...
    def test_can_create_badgeclass_with_serverAdmin_token(self):
        ...
        self.setup_user(authenticate=True, verified=True, token_scope="rw:serverAdmin")
    ...
    def test_update_badgeclass_with_null_criteria_v2_bad_request(self):
        ...
        response1 = self.client.put(
            "/v2/badgeclasses/{badge}".format(badge=modifyData.get("entityId")),
            data=modifyData,
            format="json",
        )
        self.assertEqual(response1.status_code, 400)
        self.assertEqual(
            response1.data["validationErrors"][0],
        ...
        self.assertEqual(
            response3.data["fieldErrors"]["criteriaNarrative"][0],
    ...
    def test_update_badgeclass_with_null_criteria_v1_bad_request(self):
        ...
        response1 = self.client.put(
            "/v1/issuer/issuers/{issuer}/badges/{badge}".format(
                issuer=test_issuer.entity_id, badge=modifyData["slug"]
            ),
            data=modifyData,
            format="json",
        )
...
class BadgeClassesChangedApplicationTests(SetupIssuerHelper, BadgrTestCase):
    def test_application_can_get_changed_badgeclasses(self):
        ...
        response = self.client.get(
            "/v2/badgeclasses/changed?since={}".format(quote_plus(timestamp))
        )
        self.assertEqual(response.status_code, 200)
