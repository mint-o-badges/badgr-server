...
class Issuer(
    ...
    def save(self, *args, **kwargs):
        ...
        if self.__original_address:
            if (
                self.street != self.__original_address["street"]
        ...
        if self.__original_verified == False and self.verified:  # noqa E712
    ...
    def ensure_owner(self):
        ...
        if self.staff.filter(
            issuerstaff__role=IssuerStaff.ROLE_OWNER, issuerstaff__user=self.created_by
        ):
            return
    ...
    def notify_issuer_owner(self, badgr_app=None, renotify=False):
        ...
        try:
            email_context = {
                ...
                # (keeps whitespaces, digits and alphabetical characters )
                "issuer_name": re.sub(r"[^\w\s]+", "", self.name, 0, re.I),
                "issuer_url": self.url,
