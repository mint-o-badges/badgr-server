...
class Command(BaseCommand):
    help = """
        Update the category extensions of existing learningpath
        participation badges to the new learningpath category
    """
    ...
    def handle(self, *args, **options):
            ...
            for badgeclass in badgeclasses:
                ...
                if category_extension is not None:
                    ...
                    if dry_run:
                        ...
                        self.stdout.write(
                            f"DRY-RUN: Logged changes for badgeclass {badgeclass.name}"
                        )
                    else:
        ...
        if dry_run and changes_log:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully wrote changes to {output_file}")
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"Failed to write to {output_file}: {str(e)}")
                )
