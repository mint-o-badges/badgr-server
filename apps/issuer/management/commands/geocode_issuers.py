...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if force:
            ...
            self.stdout.write(f"Processing {issuers.count()} issuers without coordinates")
        ...
        for issuer in issuers:
            ...
            try:
                ...
                if geoloc:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Success: {geoloc.latitude}, {geoloc.longitude}")
                    )
                    successful += 1
                else:
                    self.stdout.write(
                        self.style.WARNING(f"✗ No geocoding result found")
                    )
                    failed += 1
