...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "Processing {} badgeclasses missing images...".format(report["total"])
        )
            ...
            try:
                ...
                if remote_image_url:
                    try:
                        ...
                        self.stdout.write(
                            "IOError fetching '{}': {}".format(remote_image_url, str(e))
                        )
                        report["ioerrors"].append((remote_image_url, str(e)))
                    else:
                        report["status_codes"][status_code] = report[
                            "status_codes"
                        ].get(status_code, []) + [remote_image_url]
                        if status_code == 200:
                            badgeclass.image = image
                            badgeclass.save()
                            report["saved"] += 1
                            self.stdout.write(
                                "Saved missing image for badgeclass(pk={}) from '{}'".format(
                            ...
                            self.stdout.write(
                                "Http error fetching '{}': {}".format(
                                    remote_image_url, status_code
                                )
                            )
                else:
                    report["no_image_url"].append(badgeclass.pk)
                    self.stdout.write(
                        "Unable to determine an image url for badgeclass(pk={})".format(
                            badgeclass.pk
                        )
                    )
