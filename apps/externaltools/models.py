...
class ExternalTool(BaseAuditedModel, BaseVersionedEntity):
    ...
    def get_launchpoint(self, launchpoint_name):
        try:
            return next(
                lp
                for lp in self.cached_launchpoints()
                if lp.launchpoint == launchpoint_name
            )
...
class ExternalToolLaunchpoint(cachemodel.CacheModel):
    externaltool = models.ForeignKey(
        "externaltools.ExternalTool", on_delete=models.CASCADE
    )
        ...
        if self.launchpoint in ["issuer_assertion_action", "earner_assertion_action"]:
            ...
            if obj:
                launch_data.update(
                    ...
                    custom_badgr_badgeclass_name=urllib.parse.quote_plus(
                        obj.cached_badgeclass.name.encode("utf-8")
                    ),
                    custom_badgr_issuer_id=obj.cached_issuer.entity_id,
                    custom_badgr_issuer_name=urllib.parse.quote_plus(
                        obj.cached_issuer.name.encode("utf-8")
                    ),
                )
                if any(
                    s.user.id == user.id for s in obj.cached_issuer.cached_issuerstaff()
                ):
                    roles.append("issuer")
    ...
    def generate_launch_data(
        self, user=None, context_id=None, **additional_launch_data
    ):
        params = dict(
        ...
        if user is not None:
            params.update(
                dict(
                    ...
                    lis_person_name_family=urllib.parse.quote_plus(
                        user.last_name.encode("utf-8")
                    ),
                    lis_person_name_given=urllib.parse.quote_plus(
                        user.first_name.encode("utf-8")
                    ),
                    lis_person_contact_email_primary=urllib.parse.quote_plus(
