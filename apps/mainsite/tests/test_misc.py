...
from mainsite.models import AccessTokenProxy, AccessTokenScope, BadgrApp
...
class TestDateSerialization(BadgrTestCase):
    ...
    def test_date_serialization(self):
        utc_date = self.TestHolder(
            timezone.datetime(2019, 12, 6, 12, 0, tzinfo=pytz.utc)
        )
        la_date = self.TestHolder(
            pytz.timezone("America/Los_Angeles").localize(
                timezone.datetime(2019, 12, 6, 12, 0)
            )
        )  # -8 hours
        ny_date = self.TestHolder(
...
class TestTokenDenorm(BadgrTestCase, SetupIssuerHelper):
    def test_scopes_created(self):
        self.setup_user(
            email="foo@bar.com", authenticate=True, token_scope="rw:backpack r:profile"
        )
...
@override_settings(
    HTTP_ORIGIN="http://api.testserver", ACCOUNT_EMAIL_CONFIRMATION_HMAC=True
)
class TestSignup(BadgrTestCase):
    def test_user_signup_email_confirmation_redirect(self):
        ...
        url_match = re.search(
            r"{}(/v1/user/confirmemail.*)".format(http_origin), mail.outbox[0].body
        )
        self.assertIsNotNone(url_match)
        confirm_url = url_match.group(1)

        expected_redirect_url = (
            "{badgrapp_redirect}{first_name}?authToken={auth}&email={email}".format(
...
class TestEmailCleanupCommand(BadgrTestCase):
    def test_email_added_for_user_missing_one(self):
        user = BadgeUser(
            email="newtest@example.com", first_name="Test", last_name="User"
        )
        user.save()
        self.assertFalse(CachedEmailAddress.objects.filter(user=user).exists())

        user2 = BadgeUser(
            email="newtest2@example.com", first_name="Test2", last_name="User"
        )
        user2.save()
        email2 = CachedEmailAddress(
            user=user2, email="newtest2@example.com", verified=False, primary=True
        )
        email2.save()
    ...
    def test_unverified_unprimary_email_sends_confirmation(self):
        ...
        email = CachedEmailAddress(
            email=user.email, user=user, verified=False, primary=False
        )
        email.save()

        user2 = BadgeUser(
            email="newtest@example.com", first_name="Error", last_name="User"
        )
        user2.save()
...
class TestBlacklist(BadgrTestCase):
    ...
    def test_blacklist_api_query_is_in_blacklist(self):
        ...
        responses.add(
            responses.GET,
            "http://example.com?id=" + blacklist.generate_hash(id_type, id_value),
            body='{"msg": "ok"}',
            status=200,
        )
    ...
    def test_blacklist_assertion_to_recipient_in_blacklist(self):
        id_type, id_value = self.Inputs[0]
        with mock.patch(
            "mainsite.blacklist.api_query_is_in_blacklist", new=lambda a, b: True
        ):
    ...
    def test_blacklist_not_configured_throws_exception(self):
        id_type, id_value = self.Inputs[1]
        with mock.patch(
            "mainsite.blacklist.api_query_recipient_id", new=lambda a, b, c, d: None
        ):
...
class TestRemoteFileToStorage(SetupIssuerHelper, BadgrTestCase):
    ...
    def tearDown(self):
        dir = os.path.join(
            "{base_url}/{upload_to}/cached/".format(
                base_url=default_storage.location, upload_to=self.test_uploaded_path
            )
        )
    ...
    def test_scrubs_hacked_svg(self):
        ...
        responses.add(responses.GET, self.test_url, body=hacked_svg, status=200)
        ...
        saved_svg_path = os.path.join(
            "{base_url}/{file_name}".format(
                base_url=default_storage.location, file_name=storage_name
            )
        )
    ...
    @responses.activate
    def test_fetch_remote_file_to_storage_with_an_unsupported_mimetype_throws_UnsupportedMediaType(
        self,
    ):
        responses.add(
            responses.GET,
