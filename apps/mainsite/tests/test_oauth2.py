...
class OAuth2TokenTests(SetupIssuerHelper, BadgrTestCase):
    ...
    def test_client_credentials_can_get_token(self):
        ...
        ApplicationInfo.objects.create(
            application=application, allowed_scopes="rw:issuer"
        )
    ...
    def test_client_credentials_400_for_get_request(self):
        ...
        response = self.client.post(
            set_url_query_params(reverse("oauth2_provider_token"), **request_data),
            data=request_data,
        )
    ...
    def test_token_request_with_query_parmas_returns_400(self):
        response = self.client.post(
            '/o/token?username="testuser"&password="testpassword"'
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_can_use_authcode_exchange(self):
        ...
        response = self.client.post(
            reverse("oauth2_code_exchange"), dict(code=authcode)
        )
        self.assertEqual(response.status_code, 200)
        self.assertDictContainsSubset(
            {"access_token": accesstoken.token}, response.data
        )

        # cant exchange invalid authcode
        response = self.client.post(
            reverse("oauth2_code_exchange"), dict(code="InvalidAuthCode")
        )
        self.assertEqual(response.status_code, 400)
        ...
        response = self.client.post(
            reverse("oauth2_code_exchange"), dict(code=expired_authcode)
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_extra_badge_connect_data_is_exposed(self):
        ...
        accesstoken = AccessTokenProxy.objects.generate_new_token_for_user(
            test_user, application=application
        )
        self.client.credentials(
            HTTP_AUTHORIZATION="Bearer {}".format(accesstoken.token)
        )
    ...
    @override_settings(
        TOKEN_BACKOFF_MAXIMUM_SECONDS=3600, TOKEN_BACKOFF_PERIOD_SECONDS=2
    )
    def test_can_reset_failed_login_backoff(self):
        ...
        user = self.setup_user(
            authenticate=False, password=password, email="testemail233@example.test"
        )
        ...
        self.assertGreaterEqual(
            backoff_data["until"], backoff_time, "backoff time should not increase."
        )
    ...
    def test_authorize_handles_authenticated_or_anonymous_or_expired_user(self):
        ...
        # expired user
        self.client.credentials(
            HTTP_AUTHORIZATION="Bearer {}".format(expiredToken.token)
        )
        response = self.client.post("/o/authorize", data=request_data)
        self.assertEqual(response.status_code, 401)
