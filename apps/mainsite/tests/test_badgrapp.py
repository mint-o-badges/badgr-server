...
from mainsite.tests.base import (
...
class TestBadgrApp(
    SetupUserHelper, SetupIssuerHelper, APITransactionTestCase, CachingTestCase
):
    def test_badgr_app_unique_default(self):
        ba_one = BadgrApp.objects.create(
            cors="one.example.com", signup_redirect="https://one.example.com/start"
        )
        ba_two = BadgrApp.objects.create(
            cors="two.example.com", signup_redirect="https://two.example.com/start"
        )

        self.assertTrue(
            ba_one.is_default,
            "The first BadgrApp created is going to be the default one",
        )
        ...
        self.assertFalse(
            ba_one.is_default, "After setting #2 to default, #1 is no longer default."
        )
