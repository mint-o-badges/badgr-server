...
from mainsite.oauth_validator import BadgrOauthServer, BadgrRequestValidator
...
from oauth2_provider.views import (
    RevokeTokenView as OAuth2ProviderRevokeTokenView,
    TokenView as OAuth2ProviderTokenView,
)
...
# this method allows users that are authorized (but do not have to be admins) to register client
# credentials for their account, currently the user can only choose the name
...
class TokenView(OAuth2ProviderTokenView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if grant_type == "oidc":
            ...
            # Log out of the django session, since from now on we use token
            # authentication; the session authentication was
