...
class CollectionPDFCreator:
    ...
    def header(self, canvas, doc, userName):
        ...
        if oebLogo is not None: 
            canvas.drawImage(oebLogo, 20, 740, width=80, height=80, mask="auto", preserveAspectRatio=True)
            header_height += 80  
    ...
    def generate_pdf(self, collection, origin):
        ...
        template = PageTemplate(id='header', frames=frame, onPage=partial(self.header, userName=collection.owner.get_full_name()))
...
class BadgeCard(Flowable):
    def __init__(self, image, title, issuer, issued_on, width=6*inch, height=2*inch):
...
class PageNumCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        if self._pageNumber == page_count:
            ...
            full_text = text_before + link_text
            ...
            self.linkURL(
                ...
                rect=(x_start + text_before_width, 35, x_start + text_before_width + link_text_width, 35 + 10),
                relative=0,
                thickness=0,  
            )
            self.setLineWidth(3)
