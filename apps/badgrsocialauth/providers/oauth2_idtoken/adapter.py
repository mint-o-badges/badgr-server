...
class IdTokenOAuth2Adapter(OAuth2Adapter):
    ...
    def __init__(self, request):
        ...
        try:
            self.access_token_url = app_settings.PROVIDERS[self.provider_id][
                "access_token_url"
            ]
            self.authorize_url = app_settings.PROVIDERS[self.provider_id][
                "authorize_url"
            ]
            self.jwks_url = app_settings.PROVIDERS[self.provider_id]["jwks_url"]
            self.intended_aud = app_settings.PROVIDERS[self.provider_id].get(
                "aud", None
            )
            self.response_type = app_settings.PROVIDERS[self.provider_id].get(
                "response_type", "code"
            )
        except KeyError:
    ...
    def get_public_key(self, kid=None):
        try:
            ...
            raise OAuth2Error(
                "Couldn't get JWKS File and process it: {}".format(e.message)
            )
    ...
    def parse_token(self, data, app=None):
        ...
        if (
            claims.get("aud") is not None
            and claims.get("aud") not in intended_audiences
        ):
            raise OAuth2Error(
    ...
    def complete_login(self, request, app, token, **kwargs):
        ...
        logger.debug(
            "IdToken | IdTokenOAuth2Adapter:complete_login()"
