...
class IdTokenAccount(ProviderAccount):
    def to_str(self):
        return self.account.extra_data.get("name", super(IdTokenAccount, self).to_str())
...
class IdTokenProvider(OAuth2Provider):
    ...
    def extract_uid(self, data):
        logger.debug(
            "{} | IdTokenProvider:extract_uid().data |  {}".format(self.id, str(data))
        )
        return str(data["sub"])

    def extract_common_fields(self, data):
        logger.debug(
            "{} | IdTokenProvider:extract_common_fields().data |  {}".format(
                self.id, str(data)
            )
        )
        return {
