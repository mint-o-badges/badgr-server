...
from saml2 import BINDING_HTTP_POST, BINDING_HTTP_REDIRECT, BINDING_SOAP, config, saml
...
class SAML2Tests(BadgrTestCase):
    def setUp(self):
        ...
        self.test_files_path = os.path.join(
            TOP_DIR, "apps", "badgrsocialauth", "testfiles"
        )
        ...
        self.sp_acs_location = "http://localhost:8000/account/saml2/{}/acs/".format(
            self.config.slug
        )
    ...
    def _skip_if_xmlsec_binary_missing(self):
        ...
        if xmlsec_binary_path is None:
            self.skipTest(
                "SKIPPING: In order to test XML Signing, XMLSEC_BINARY_PATH to xmlsec1 must be configured."
            )
    ...
    def _initiate_login(self, idp_name, badgr_app, user=None):
        ...
        if user is not None:
            ...
            )  # strip server info from location
    ...
    def test_signed_authn_request_option_creates_signed_metadata(self):
        ...
        with override_settings(
            SAML_KEY_FILE=self.ipd_key_path, SAML_CERT_FILE=self.ipd_cert_path
        ):
    ...
    def test_signed_authn_request_option_returns_self_posting_form_populated_with_signed_metadata(
        self,
    ):
        self._skip_if_xmlsec_binary_missing()
        self.config.use_signed_authn_request = True
        ...
        with override_settings(
            ...
            self.assertIsNot(
                response.content.find(
                    b'<input type="hidden" name="SAMLRequest" value="'
                ),
                -1,
            )
    ...
    def test_add_samlaccount_to_existing_user(self):
        ...
        fake_request = rf.post(
            reverse(
                "assertion_consumer_service", kwargs={"idp_name": self.config.slug}
            ),
    ...
    def test_saml2_create_account_multiple_email_already_taken(self):
        ...
        email = "exampleuser@example.com"
        self.setup_user(email=email, token_scope="rw:profile rw:issuer rw:backpack")
    ...
    def test_add_samlaccount_to_existing_user_with_varying_email(self):
        ...
        resp = auto_provision(
            fake_request, [email2], t_user.first_name, t_user.last_name, self.config
        )
        self.assertEqual(resp.status_code, 302)
        ...
        Saml2Account.objects.get(
            user=t_user
        )  # There is a Saml account associated with the user.
        CachedEmailAddress.objects.get(
            email=email2, user=t_user, verified=True, primary=False
        )  # User has the email.
        ...
        CachedEmailAddress.objects.get(
            email=email3, user=t_user, verified=True, primary=False
        )  # User has new email.
    ...
    def test_can_extract_custom_userdata(self):
        ...
        self.assertEqual(
            reloaded_config.custom_settings_data["email"],
            [],
            "default is set to an empty list",
        )
        self.assertEqual(
            reloaded_config.custom_settings_data["first_name"],
            ["customMyClientFirstName"],
        )
        ...
        self.assertEqual(
            userdata_from_saml_assertion(
                fake_saml_assertion, "email", config=reloaded_config
            ),
            fake_saml_assertion["emailaddress"][0],
        )
        self.assertEqual(
            userdata_from_saml_assertion(
                fake_saml_assertion, "first_name", config=reloaded_config
            ),
            fake_saml_assertion["customMyClientFirstName"][0],
        )
        self.assertEqual(
            userdata_from_saml_assertion(
                fake_saml_assertion, "last_name", config=reloaded_config
            ),
    ...
    def create_authn_response(
        ...
        **kwargs,
        ...
        try:
            _authn = authn
            if (
                sign_assertion or sign_response
            ) and self.sec.cert_handler.generate_cert():
                with self.lock:
