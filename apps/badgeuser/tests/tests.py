...
from mainsite.models import Application, ApplicationInfo, BadgrApp
...
class UserCreateTests(BadgrTestCase):
    def test_create_user(self):
        ...
        self.badgr_app.email_confirmation_redirect = (
    ...
    def test_user_can_add_secondary_email_without_welcome_query_param(self):
        ...
        CachedEmailAddress.objects.create(
            user=first_user, email=email, primary=False, verified=False
        )
    ...
    def test_can_create_user_with_preexisting_unconfirmed_email(self):
        ...
        existing_user = self.setup_user(
            email=email, password="secret", authenticate=False, verified=False
        )
    ...
    def test_can_create_account_with_same_email_since_deleted(self):
        ...
        second_email = CachedEmailAddress(
            email=new_email, user=first_user, verified=True
        )
        second_email.save()
    ...
    def test_shouldnt_error_when_user_exists_with_email(self):
        ...
        self.setup_user(
            email=email, password="secret2"
        )  # password is set because its an existing user
    ...
    def test_autocreated_user_can_signup(self):
        ...
        self.setup_user(
            email=email, password=None, create_email_address=False
        )  # no password set
    ...
    def test_autocreated_user_signup(self):
        ...
        user = BadgeUser(email="testuser123@example.test")
        user.save()
...
class UserEmailTests(BadgrTestCase):
    def setUp(self):
        ...
        self.first_user = self.setup_user(
            email=self.first_user_email, authenticate=True
        )
        CachedEmailAddress.objects.create(
            user=self.first_user, email=self.first_user_email_secondary, verified=True
        )
        response = self.client.get("/v1/user/auth-token")
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_can_remove_email(self):
        ...
        not_primary = random.choice(
            [e for e in response.data if e["verified"] and not e["primary"]]
        )
    ...
    def test_user_can_make_email_primary(self):
        ...
        response = self.client.put(
            "/v1/user/emails/{}".format(not_primary.get("id")), {"primary": True}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_can_resend_verification_email(self):
        ...
        response = self.client.put(
            "/v1/user/emails/{}".format(not_verified.get("id")), {"resend": True}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_can_request_forgot_password(self):
        ...
        self.assertEqual(
            response.status_code, 200, "Does not leak information about account emails"
        )
        self.assertEqual(len(mail.outbox), 0)

        # successfully send recovery email
        response = self.client.post(
            "/v1/user/forgot-password", {"email": self.first_user_email}
        )
        ...
        self.assertEqual(len(mail.outbox), 1)
        matches = re.search(
            r"/v1/user/forgot-password\?token=([-0-9a-zA-Z]*)", mail.outbox[0].body
        )
        ...
        response = self.client.put(
            "/v1/user/forgot-password", {"token": token, "password": new_password}
        )
        self.assertEqual(response.status_code, 200)
        ...
        ApplicationInfo.objects.create(
            application=application, allowed_scopes="rw:issuer rw:backpack rw:profile"
        )
    ...
    def test_log_when_api_auth_token_endpoint_is_used(self, mocked_logger):
        ...
        self.assertIn(
            ...
            "There is a warning returned to the requester",
        )
    ...
    def test_log_when_legacy_auth_token_is_used(self, mocked_logger):
        ...
        self.client.get(
            "/v2/users/self", HTTP_AUTHORIZATION="Token {}".format(token.key)
        )

        mocked_logger.assert_called_once()
        self.assertIsNotNone(
            mocked_logger.call_args[0][0].request.META.get("REMOTE_ADDR", None)
        )
    ...
    def test_can_create_new_variant_api(self):
        ...
        response = self.client.post(
            "/v1/user/emails", {"email": "HelloAgain@world.com"}
        )
    ...
    def test_can_create_variants(self):
        ...
        first_email = CachedEmailAddress.objects.create(
            email="test@example.com", verified=True, user=user
        )
        self.assertIsNotNone(first_email.pk)
        ...
        first_variant = EmailAddressVariant(
            email=first_variant_email, canonical_email=first_email
        )
        first_variant.save()
    ...
    def test_user_can_create_variant_method(self):
        ...
        self.assertFalse(
            user.can_add_variant("howdyfeller@world.com")
    ...
    def test_can_create_variant_for_unconfirmed_email(self):
        ...
        new_email = CachedEmailAddress.objects.create(
            email=new_email_address, user=user
        )
        new_variant = EmailAddressVariant(
            email=new_email_address.upper(), canonical_email=new_email
        )
        ...
        verified_emails = [
            e.email for e in user.emailaddress_set.filter(verified=True)
...
class UserRecipientIdentifierTests(SetupIssuerHelper, BadgrTestCase):
    ...
    def test_recipient_identity_serialized_to_correct_fields(self):
        ...
        CachedEmailAddress.objects.create(
            user=user, email=primary_email, primary=True, verified=True
        )
        v2serialized = BadgeUserSerializerV2(user).data["result"][0]
    ...
    def test_verified_recipient_receives_assertion(self):
        ...
        self.badgeclass.issue(
            recipient_id=url, recipient_type=UserRecipientIdentifier.IDENTIFIER_TYPE_URL
        )
...
class UserBadgeTests(BadgrTestCase):
    ...
    def create_badgeclass(self):
        with open(
            os.path.join(
                TOP_DIR, "apps", "issuer", "testfiles", "guinea_pig_testing_badge.png"
            ),
            "rb",
        ) as fh:
            ...
            badgeclass = BadgeClass.objects.create(
                ...
                image=SimpleUploadedFile(
                    name="test_image.png", content=fh.read(), content_type="image/png"
                ),
            )
            return badgeclass
    ...
    def test_badge_awards_transferred_on_email_verification(self):
        ...
        badgeclass.issue(
            recipient_id="New+email@newemail.com",
        ...
        verify_url = re.search("(?P<url>/v1/[^\\s]+)", mail.outbox[-1].body).group(
            "url"
        )
        response = self.client.get(verify_url)
        self.assertEqual(response.status_code, 302)
        ...
        self.assertTrue(
            "New+email@newemail.com" in [e.email for e in email.cached_variants()]
        )
        self.assertTrue(
            "New+Email@newemail.com" in [e.email for e in email.cached_variants()]
        )
...
class UserProfileTests(BadgrTestCase):
    ...
    def test_user_can_change_profile(self):
        ...
        response = self.client.put(
            "/v1/user/profile", {"first_name": "Barry", "last_name": "Manilow"}
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.put(
            "/v1/user/profile",
            {"password": third_password, "current_password": new_password},
        )
        self.assertEqual(response.status_code, 200)
        self.client.logout()
    ...
    def test_user_update_ignores_blank_email(self):
        ...
        response = self.client.put(
            "/v1/user/profile",
            {"first_name": first + " Q.", "last_name": last, "email": None},
            format="json",
        )
        self.assertEqual(response.status_code, 200)
