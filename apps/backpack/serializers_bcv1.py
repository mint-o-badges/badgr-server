...
from mainsite.serializers import HumanReadableBooleanField, MarkdownCharField
...
class BadgeConnectApiInfoSerializer(serializers.Serializer):
    ...
    scopesRequested = serializers.ListField(
        read_only=True, child=serializers.URLField()
    )
...
class BadgeConnectBaseEntitySerializer(serializers.Serializer):
    def to_representation(self, instance):
        representation = super(
            BadgeConnectBaseEntitySerializer, self
    ...
    def create(self, validated_data):
        ...
        try:
            instance, created = BadgeCheckHelper.get_or_create_assertion(
                url=url, created_by=self.context["request"].user
            )
            if not created:
