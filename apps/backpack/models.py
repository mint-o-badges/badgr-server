...
from issuer.models import BadgeInstance, BaseAuditedModelDeletedWithUser
from issuer.utils import CURRENT_OBI_VERSION, add_obi_version_ifneeded, get_obi_context
...
class BackpackCollection(BaseAuditedModelDeletedWithUser, BaseVersionedEntity):
    ...
    def cached_badgeinstances(self):
        return self.assertions.filter(
            revoked=False,
            acceptance__in=(
                BadgeInstance.ACCEPTANCE_ACCEPTED,
    ...
    def share_url(self):
        if self.published:
            return OriginSetting.HTTP + reverse(
                "collection_json", kwargs={"entity_id": self.share_hash}
            )
            ...
            for badge_reference in value:
                try:
                    if isinstance(badge_reference, BadgeInstance):
                        ...
                        badgeinstance = BadgeInstance.cached.get(
                            entity_id=badge_reference
                        )
                    ...
                    if badgeinstance.entity_id not in list(existing_badges.keys()):
                        BackpackCollectionBadgeInstance.cached.get_or_create(
                            collection=self, badgeinstance=badgeinstance
                        )
    ...
    def get_json(
        ...
        json["badges"] = [
            ...
            for b in self.cached_badgeinstances()
        ]
...
class BackpackCollectionBadgeInstance(cachemodel.CacheModel):
    collection = models.ForeignKey(
        "backpack.BackpackCollection", on_delete=models.CASCADE
    )
...
class BackpackBadgeShare(BaseSharedModel):
    badgeinstance = models.ForeignKey(
        "issuer.BadgeInstance", null=True, on_delete=models.CASCADE
    )
...
class BackpackCollectionShare(BaseSharedModel):
    collection = models.ForeignKey(
        "backpack.BackpackCollection", null=False, on_delete=models.CASCADE
    )
