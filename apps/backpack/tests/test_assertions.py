...
from mainsite.utils import OriginSetting, first_node_match
...
class TestShareProviders(SetupIssuerHelper, BadgrTestCase):
    ...
    # name with ascii
    badge_class_name_non_ascii = base64.b64decode("w45zc8O8w6lycyBDb3Jw").decode(
        "utf-8"
    )
    ...
    def test_twitter_share_with_ascii_issuer(self):
        ...
        test_issuer = self.setup_issuer(
            owner=test_user, name=self.issuer_name_non_ascii
        )
        test_badgeclass = self.setup_badgeclass(issuer=test_issuer)
        test_assertion = test_badgeclass.issue(recipient_id="new.recipient@email.test")
        share = BackpackBadgeShare(
            provider=provider, badgeinstance=test_assertion, source="unknown"
        )
    ...
    def test_pintrest_share_with_ascii_summary(self):
        ...
        test_badgeclass = self.setup_badgeclass(
            issuer=test_issuer, name=self.badge_class_name_non_ascii
        )
        test_assertion = test_badgeclass.issue(recipient_id="new.recipient@email.test")
        share = BackpackBadgeShare(
            provider=provider, badgeinstance=test_assertion, source="unknown"
    ...
    def test_unsupported_share_provider_returns_404(self):
        ...
        get_response = self.client.get(
            "/v1/earner/share/badge/{badge_id}?provider={provider}".format(
                badge_id=test_assertion.entity_id, provider=provider
            )
        )
...
class TestBadgeUploads(BadgrTestCase):
    ...
    def test_submit_basic_1_1_badge_via_url(self):
        ...
        for d in [assertion_data, badgeclass_data, issuer_data]:
            responses.add(responses.GET, d["id"], json=d)

        responses.add(
            responses.GET,
            "http://a.com/badgeclass_image",
            body=open(
                os.path.join(CURRENT_DIRECTORY, "testfiles/unbaked_image.png"), "rb"
            ).read(),
            status=200,
    ...
    def test_submit_basic_1_0_badge_from_image_url_baked_w_assertion(self):
        ...
        responses.add(
            responses.GET,
            "http://a.com/baked_image",
            body=open(
                os.path.join(CURRENT_DIRECTORY, "testfiles/baked_image.png"), "rb"
            ).read(),
            status=200,
            content_type="image/png",
        )

        post_input = {"url": "http://a.com/baked_image"}
        with mock.patch(
            "mainsite.blacklist.api_query_is_in_blacklist", new=lambda a, b: False
        ):
            response = self.client.post("/v1/earner/badges", post_input)
        ...
        setup_resources(
            [
                            ...
                            "obi:validation": [
                                {
                                    ...
                                    "obi:validationSchema": "https://openbadgespec.org/extensions/exampleExtension/schema.json",  # noqa: E501
                                }
                            ],
                        }
                    ),
                    ...
                    "response_body": json.dumps(
                        {
                            ...
                            "type": "object",
                            "properties": {"exampleProperty": {"type": "string"}},
                ...
                },
                {
                    ...
                    "response_body": json.dumps(assertion_metadata),
                },
                {
                    "url": "http://a.com/badgeclass",
                    "response_body": json.dumps(badgeclass_metadata),
                },
                {
                    "url": "http://a.com/issuer",
                    "response_body": json.dumps(issuer_metadata),
                },
            ]
        )
        self.setup_user(email="test@example.com", authenticate=True)
    ...
    def test_submit_basic_1_0_badge_assertion(self):
        ...
        post_input = {
            "assertion": open(
                os.path.join(CURRENT_DIRECTORY, "testfiles/1_0_basic_instance.json"),
                "r",
            ).read()
        }
        with mock.patch(
    ...
    def test_submit_basic_1_0_badge_url_variant_email(self):
            ...
            [
                {
                    ...
                    "filename": "1_0_basic_instance3.json",
                },
                {"url": OPENBADGES_CONTEXT_V1_URI, "filename": "v1_context.json"},
                {
                    "url": OPENBADGES_CONTEXT_V2_URI,
        ...
        response = self.client.post("/v1/user/emails", dict(email="TEST@example.com"))
        self.assertEqual(
            response.status_code, 400
        )  # adding a variant successfully returns a 400
        ...
        self.assertEqual(get_response.status_code, 200)
        self.assertEqual(
            get_response.data[0].get("json", {}).get("id"), "http://a.com/instance3"
        )
        self.assertEqual(
            get_response.data[0]
            .get("json", {})
        ...
        self.assertTrue(
            "TEST@example.com" in [e.email for e in email.cached_variants()]
        )
    ...
    def test_submit_basic_1_0_badge_with_inaccessible_badge_image(self):
        ...
        self.assertIsNotNone(
            first_node_match(
                response.data, dict(messageLevel="ERROR", name="IMAGE_VALIDATION")
            )
        )
    ...
    def test_submit_basic_1_0_badge_missing_badge_prop(self):
        ...
        responses.add(
            ...
            body=open(
                os.path.join(
                    CURRENT_DIRECTORY,
                    "testfiles/1_0_basic_instance_missing_badge_prop.json",
                ),
                "r",
            ).read(),
            status=200,
        ...
        self.assertIsNotNone(
            first_node_match(
                response.data,
                dict(messageLevel="ERROR", name="VALIDATE_PROPERTY", prop_name="badge"),
            )
        )
    ...
    def test_submit_basic_0_5_0_badge_via_url(self):
        ...
        post_input = {"url": "http://oldstyle.com/instance"}
        with mock.patch(
            "mainsite.blacklist.api_query_is_in_blacklist", new=lambda a, b: False
        ):
            response = self.client.post("/v1/earner/badges", post_input)
    ...
    def test_submit_0_5_badge_upload_by_assertion(self):
        ...
        post_input = {
            "assertion": open(
                os.path.join(CURRENT_DIRECTORY, "testfiles", "0_5_basic_instance.json"),
                "r",
            ).read()
        }
        with mock.patch(
    ...
    def test_submit_badges_with_intragraph_references(self):
        ...
        post_input = {"assertion": json.dumps(assertion)}
        with mock.patch(
            "mainsite.blacklist.api_query_is_in_blacklist", new=lambda a, b: False
        ):
            response = self.client.post("/v1/earner/badges", post_input, format="json")
...
class TestDeleteLocalAssertion(BadgrTestCase, SetupIssuerHelper):
    ...
    def test_can_delete_local(self):
        ...
        self.assertEqual(
            len(response.data), 1, "There is a badge in the recipient's backpack"
        )

        response = self.client.delete(
            "/v1/earner/badges/{}".format(assertion.entity_id)
        )
        self.assertEqual(response.status_code, 204)
        ...
        self.assertEqual(
            len(response.data),
            0,
            "There is no longer a badge in the recipient's backpack",
        )
        ...
        setup_resources(
            [
                ...
                },
                {
                    ...
                    "response_body": json.dumps(assertion.get_json()),
                },
                {
                ...
                {
                    "url": test_badgeclass.jsonld_id,
                    "response_body": json.dumps(test_badgeclass.get_json()),
                },
                {
                ...
                {
                    "url": test_issuer.jsonld_id,
                    "response_body": json.dumps(test_issuer.get_json()),
                },
            ]
        )

        post_input = {"url": assertion.jsonld_id}
        with mock.patch(
            "mainsite.blacklist.api_query_is_in_blacklist", new=lambda a, b: False
        ):
            response = self.client.post("/v1/earner/badges", post_input)
    ...
    def test_can_upload_non_hashed_url_badge(self):
        ...
        UserRecipientIdentifier.objects.create(
            user=test_recipient,
            identifier="https://twitter.com/testuser1",
            verified=True,
        ...
        setup_resources(
            [
                ...
                },
                {
                    ...
                    "response_body": assertion_data,
                },
                {
                    "url": json.loads(badgeclass_data)["id"],
                    "response_body": badgeclass_data,
                },
                {
...
class TestAcceptanceHandling(BadgrTestCase, SetupIssuerHelper):
    def test_can_accept_badge(self):
        ...
        self.setup_user(
            email="test_recipient@email.test",
            authenticate=True,
            token_scope="rw:backpack",
        )
        with mock.patch(
            "mainsite.blacklist.api_query_is_in_blacklist", new=lambda a, b: False
        ):
...
class TestExpandAssertions(BadgrTestCase, SetupIssuerHelper):
    def test_no_expands(self):
        ...
        self.assertTrue(
            not isinstance(
                response.data["result"][0]["badgeclass"], collections.OrderedDict
            )
        )
    ...
    def test_expand_badgeclass_single_assertion_single_issuer(self):
        ...
        self.assertTrue(
            not isinstance(
                response.data["result"][0]["badgeclass"]["issuer"],
        ...
        response = self.client.get(
            "/v2/backpack/assertions/{}?expand=badgeclass&expand=issuer".format(fid)
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_expand_badgeclass_and_isser_single_assertion_single_issuer(self):
        ...
        response = self.client.get(
            "/v2/backpack/assertions?expand=badgeclass&expand=issuer"
        )
    ...
    def test_expand_badgeclass_mult_assertions_mult_issuers(self):
        ...
        test_user = self.setup_user(
            email="test_recipient@email.test", authenticate=True
        )
        ...
        # define badgeclasses
        test_badgeclass_one = self.setup_badgeclass(
            name="Test Badgeclass 1", issuer=test_issuer_one
        )
        test_badgeclass_two = self.setup_badgeclass(
            name="Test Badgeclass 2", issuer=test_issuer_one
        )
        test_badgeclass_three = self.setup_badgeclass(
            name="Test Badgeclass 3", issuer=test_issuer_two
        )
        test_badgeclass_four = self.setup_badgeclass(
            name="Test Badgeclass 4", issuer=test_issuer_three
        )
        test_badgeclass_five = self.setup_badgeclass(
            name="Test Badgeclass 5", issuer=test_issuer_three
        )
        test_badgeclass_six = self.setup_badgeclass(
            name="Test Badgeclass 6", issuer=test_issuer_three
        )
        ...
        for i in range(6):
            self.assertTrue(
                isinstance(
                    response.data["result"][i]["badgeclass"], collections.OrderedDict
                )
            )
            self.assertTrue(
                not isinstance(
                    response.data["result"][i]["badgeclass"]["issuer"],
                    collections.OrderedDict,
                )
            )
...
class TestPendingBadges(BadgrTestCase, SetupIssuerHelper):
    ...
    def test_view_badge_i_imported(self):
        ...
        with mock.patch(
            ...
            post_resp = self.client.post(
                "/v2/backpack/import", post_input, format="json"
            )
        ...
        get_resp = self.client.get(
            "/v1/earner/badges?json_format=plain&include_pending=1"
        )
        ...
        get_resp = self.client.get(
            "/v1/earner/badges?json_format=plain&include_pending=0"
        )
    ...
    def test_view_badge_i_imported_with_v1(self):
        ...
        get_resp3 = self.client.get(
            "/v1/earner/badges?json_format=plain&include_pending=1"
        )
        self.assertEqual(len(get_resp3.data), 1)

        get_resp4 = self.client.get(
            "/v1/earner/badges?json_format=plain&include_pending=false"
        )
    ...
    def test_cant_view_badge_awarded_to_unverified_that_i_did_not_import(self):
        ...
        test_badgeclass_one.issue(
            recipient_id="test@example.com", recipient_type="email"
        )
        get_resp = self.client.get("/v2/backpack/assertions?include_pending=1")
...
class TestInclusionFlags(BadgrTestCase, SetupIssuerHelper):
    def test_include_revoked(self):
        ...
        revoked_assertion = test_badgeclass_one.issue(
            recipient_id="test@example.com", recipient_type="email"
        )
        ...
        self.assertEqual(result.status_code, 200)
        self.assertEqual(
            len(result.data), 1, "V1 Backpack defaults to false for these revoked"
        )
    ...
    def test_include_expired(self):
        ...
        expired_assertion = test_badgeclass_one.issue(
            recipient_id="test@example.com", recipient_type="email"
        )
        ...
        self.assertEqual(result.status_code, 200)
        self.assertEqual(
            len(result.data), 2, "V1 Backpack defaults to true for these values"
        )
    ...
    def test_include_expired_and_revoked(self):
        ...
        result = self.client.get(
            "/v2/backpack/assertions?include_expired=1&include_revoked=1"
        )
        self.assertEqual(result.status_code, 200)
        ...
        self.assertEqual(
            len(result.data),
            2,
            "V1 Backpack defaults to true for expired but not revoked",
        )
