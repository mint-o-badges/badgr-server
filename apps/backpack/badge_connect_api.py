...
from backpack.serializers_bcv1 import (
    ...
    BadgeConnectManifestSerializer,
)
        ...
        "badgeConnectAPI": [
            {
                ...
                "registrationUrl": "{}{}".format(
                    settings.HTTP_ORIGIN, reverse("oauth2_api_register")
                ),
                "authorizationUrl": "https://{}/auth/oauth2/authorize".format(domain),
                "tokenUrl": "{}{}".format(
                    settings.HTTP_ORIGIN, reverse("oauth2_provider_token")
                ),
            }
        ],
    }
...
class BadgeConnectManifestRedirectView(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        ...
        return settings.HTTP_ORIGIN + reverse(
            "badge_connect_manifest", kwargs={"domain": badgr_app.cors}
        )
...
class BadgeConnectAssertionListView(ListCreateAPIView):
    ...
    def get_queryset(self):
        qs = BadgeInstance.objects.filter(
            recipient_identifier__in=self.request.user.all_recipient_identifiers
        ).order_by("-updated_at")
        if self.request.query_params.get("since", None):
            qs = qs.filter(
                updated_at__gte=parse_datetime(self.request.query_params.get("since"))
            )
        return qs
